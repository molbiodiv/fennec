<h3 class="page-header">
    Popular trait types
</h3>
<div class="row">
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
        <div class="panel panel-trait">
            <div class="panel-heading">
                <div class="row">
                    <div class="col-xs-3">
                        <img src="{#$WebRoot#}/css/img/habitat.png" height="100px" style="float: left">
                    </div>
                    <div class="col-xs-9 text-right">
                        <div class="huge">188341</div>
                        <div>Habitat</div>
                    </div>
                </div>
            </div>
            <a href="{#$WebRoot#}/project">
                <div class="panel-footer info-trait">
                    <span class="pull-left">View Details</span>
                    <span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
                    <div class="clearfix"></div>
                </div>
            </a>
        </div>
    </div>
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
        <div class="panel panel-trait">
            <div class="panel-heading">
                <div class="row">
                    <div class="col-xs-3">
                        <img src="{#$WebRoot#}/css/img/geographicDist.png" height="100px" style="float: left">
                    </div>
                    <div class="col-xs-9 text-right">
                        <div class="huge">134111</div>
                        <div>Geographic Distribution</div>
                    </div>
                </div>
            </div>
            <a href="{#$WebRoot#}/project">
                <div class="panel-footer info-trait">
                    <span class="pull-left">View Details</span>
                    <span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
                    <div class="clearfix"></div>
                </div>
            </a>
        </div>
    </div>
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
        <div class="panel panel-trait">
            <div class="panel-heading">
                <div class="row">
                    <div class="col-xs-3">
                        <img src="{#$WebRoot#}/css/img/plantHeight.png" height="100px" style="float:left">
                    </div>
                    <div class="col-xs-9 text-right">
                        <div class="huge">95460</div>
                        <div>Plant Height</div>
                    </div>
                </div>
            </div>
            <a href="{#$WebRoot#}/project">
                <div class="panel-footer info-trait">
                    <span class="pull-left">View Details</span>
                    <span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
                    <div class="clearfix"></div>
                </div>
            </a>
        </div>
    </div>
</div>            
<div class="row">
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
        <div class="panel panel-trait">
            <div class="panel-heading">
                <div class="row">
                    <div class="col-xs-3">
                        <img src="{#$WebRoot#}/css/img/seawaterTmp.png" height="100px" style="float:left">
                    </div>
                    <div class="col-xs-9 text-right">
                        <div class="huge">92540</div>
                        <div>Seawater temperature</div>
                    </div>
                </div>
            </div>
            <a href="{#$WebRoot#}/project">
                <div class="panel-footer info-trait">
                    <span class="pull-left">View Details</span>
                    <span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
                    <div class="clearfix"></div>
                </div>
            </a>
        </div>
    </div>
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
        <div class="panel panel-trait">
            <div class="panel-heading">
                <div class="row">
                    <div class="col-xs-3">
                        <img src="{#$WebRoot#}/css/img/weight.png" height="100px" style="float:left">
                    </div>
                    <div class="col-xs-9 text-right">
                        <div class="huge">71042</div>
                        <div>Body Mass</div>
                    </div>
                </div>
            </div>
            <a href="{#$WebRoot#}/project">
                <div class="panel-footer info-trait">
                    <span class="pull-left">View Details</span>
                    <span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
                    <div class="clearfix"></div>
                </div>
            </a>
        </div>
    </div>
    <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
        <div class="panel panel-trait">
            <div class="panel-heading">
                <div class="row">
                    <div class="col-xs-3">
                        <img src="{#$WebRoot#}/css/img/plantColor.png" height="100px" style="float:left">
                    </div>
                    <div class="col-xs-9 text-right">
                        <div class="huge">69540</div>
                        <div>Flower color</div>
                    </div>
                </div>
            </div>
            <a href="{#$WebRoot#}/project">
                <div class="panel-footer info-trait">
                    <span class="pull-left">View Details</span>
                    <span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>
                    <div class="clearfix"></div>
                </div>
            </a>
        </div>
    </div>
</div>
<h4 class="page-header"></h4>
<div class="row">
    <div class="col-lg-12">
        <a class="btn btn-lg btn-trait-search" href="{#$WebRoot#}/trait/search" role="button">Search for Traits</a>
    </div>
</div>