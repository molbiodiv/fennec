...
(function() {
  ...
  init = (function() {
    ...
    function init(page) {
      this.saveIndexedDb = bind(this.saveIndexedDb, this);
      this.viz = bind(this.viz, this);
      console.log(page);
      if (this.mobilecheck()) {
        $('body').css({
    ...
    init.prototype.mobilecheck = function() {
      var check;
      check = false;
      (function(a) {
    ...
    init.prototype.viz = function() {
      var hostURL, name, ref, regex, results, shareURL;
      name = 'shareID';
      regex = new RegExp("[\\?&]" + name + "=([^&#]*)");
      results = regex.exec(location.search);
            if ((ref = results === null) != null) {
        ref;
      } else {
        ({
      ...
      if (results !== null) {
        ...
        return $.getJSON(shareURL, (function(_this) {
          return function(shareJSON) {
            var optionJSON, shareFile;
            ...
            return $.get(shareFile, function(data) {
              var w;
              w = new Worker('scripts/unzipWorker.js');
              w.addEventListener('message', function(e) {
                return _this.saveIndexedDb(e.data, shareJSON, optionJSON);
              });
              return w.postMessage(data);
            }).fail(function() {
              return alert("This shared link no long exists ...");
            });
          };
        })(this));
      } else {
        return $("#GraphGallery .col3").each(function(index) {
          return $(this).click(function() {
            var app, title;
            if (index === 5) {
              return alert('Implementing... ');
            } else {
              $('#loadingIcon').css('opacity', '1');
              $('#goBackFilter').fadeOut(200);
              title = $(this).find('p').text();
              $('#GraphGallery').fadeOut(300, function() {
                $('#up_sec').fadeIn(300);
                return $('.pageName').html(title).css('margin-top', '-74px');
              });
              return app = new taxonomyViz(index + 1, 2);
            }
          });
        });
      }
    ...
    init.prototype.saveIndexedDb = function(shareBiom, shareJSON, optionJSON) {
      var biomToStore, d;
      d = new Date();
      biomToStore = {
        name: optionJSON.name,
        size: shareBiom.length,
        data: shareBiom,
        date: d.getUTCFullYear() + "-" + (d.getUTCMonth() + 1) + "-" + d.getUTCDate() + "T" + d.getUTCHours() + ":" + d.getUTCMinutes() + ":" + d.getUTCSeconds() + " UTC"
      };
      return db.open({
        server: "BiomData",
        version: 1,
        ...
        return function(s) {
          return s.biom.add(biomToStore).done(function() {
            ...
            return db.open({
              ...
              return function(t) {
                ...
                return t.biomSample.add(optionJSON).done(function(item) {
                  console.log('option json uploaded!');
                  $('#shareGO').html('GO >>');
                  return $('#shareGO').click(function() {
                    var app, title;
                    ...
                    return app = new taxonomyViz(parseInt(shareJSON.visualization_id), parseInt(shareJSON.layer_id));
                  });
                });
              };
            })(this));
    ...
    init.prototype.helpMenu = function() {
      return $('#help').click(function(e) {
        e.preventDefault();
        $('#dialog-modal').show();
        $('#dialog-modal').dialog({
          height: 140,
        ...
        return $('.ui-button-text').html('<i class="icon-remove icon-large"></i>');
      });
    };
